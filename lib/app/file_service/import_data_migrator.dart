...
class ImportDataMigrator {
  ...
  Future<Map<String, dynamic>> migrate(
    ...
    final migratedJson = _migrateJsonData(jsonData);
  ...
  Map<String, dynamic> _migrateJsonData(Map<String, dynamic> jsonData) {
    final version = jsonData[versionKey] as int? ?? 1;
  ...
  static Future<void> _migrateFileStructureIsolate(String extractDirPath) async {
    ...
    for (final entityDir in entities) {
      ...
      if (entityName == "collections" || int.tryParse(entityName) == null) continue;

      final newPath = p.join(filmsDir.path, entityName);
      try {
